        ...
        <div v-for="(data, index) in productsOrder"  :key="index" class="order_product">
            <ul class="order_product-list">
                <li>
                    ...
                    <div  class="change">
                        <div  @click="delivery(data)" :class="{ 'disabled': data.status === 'Đang giao hàng' }" class="status">{{ data.status }}</div>
                        <div @click="deleteOrder(data, index)" class="delete">Hủy đơn</div>
                    </div>
                </li>
            </ul>
        </div>
...
<script>
    ...
    export default{
        ...
        return {
            ...
            user_name,cart, selectAddress,productsOrder, deleteOrder, backPage, delivery
        }
    },
    }
</script>
