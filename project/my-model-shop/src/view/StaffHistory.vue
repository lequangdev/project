                ...
                <ul class="order_product-list">
                    <li>
                        ...
                        <div class="product_price">
                            <span class="tags-price">Giá bán: {{ data.price }}</span>
                        </div>
                        <div class="status" :class="statusClass(data.status)">{{  statusText(data.status) }}</div>
                    </li>
                </ul>

            </div>
            <div class="btnback" @click="backPage">Quay lại </div>
            <div class="home" @click="pageHome"> Trang chủ </div>
        </div>
    </div>
</template>
...
export default {
    ...
    setup() {
        ...
        let store = useStore()
        let productsOrder = ref([])
        const router = useRouter()
        const introduce = 'WANG'
        const title = 'Lịch sử giao hàng' 
        function pageHome(){
            router.push("/")
        }
        function getStaffHistory(){
            axios({
                method: "get",
                url: 'http://localhost:1234/api/Historydelivery/getStaffHistory', 
                params: {
                    staff_id: localStorage.getItem("user_id")
                } 
            })
            .then(response => {
                productsOrder.value = response.data
            })
            .catch(error => {
        ...
        const statusClass = (status) => {
            return {
                'status-delivered': status === '1',
                'status-error': status === '2',
                'status-refuse': status === '3'
            }
        }
        return {
            introduce, title,
            productsOrder, pageHome, backPage, statusText, statusClass, avatar, onProfileUser
        }
    },
    components: {
        HeaderVue
